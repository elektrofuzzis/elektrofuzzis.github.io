...
Die ftSwarm-Firmware hat eigene Log-Funktionen. Diese können Sie auch zum Debugging Ihres Modells verwenden. Es stehen 5 Debuggingfunktionen zur Verfügung:

- **SWARM_LOG_INFO** Schreibt eine [INFO] Meldung.
- **SWARM_LOG_WARN** Schreibt eine [WARINING] Meldung.
- **SWARM_LOG_WAIT** Schreibt eine [WAITING] Meldung und setzt die LEDs auf blau - warte auf Hardware.
- **SWARM_LOG_ERROR** Schreibt eine [ERROR] Meldung und setzt die LEDs auf rot - Fehler.
- **SWARM_LOG_FATAL** Schreibt eine [FATAL] Meldung inklusive des Filenamens und der Quelltextzeile. Alle Motoren werden angehalten und die Programmausführung gestoppt. Die LEDs werden auf pink gesetzt.

```cpp
SWARM_LOG_INFO("SWARM_LOG_INFO funktioniert wie printf. %d*%d=%s", 6, 7, "zweiundvierzig");

Ausgabe:
[INFO] SWARM_LOG_INFO funktioniert wie printf. 6*7=zweiundvierzig
```
