...
function getLanguage(availableLanguages) {
  ...
  async function aggressivePortCleanup() {
    ...
    // 1. ESPLoader und Transport-Referenzen trennen
    if (esploader) {
      try {
        // Falls der ESPLoader/Transport eine eigene Disconnect-Routine hat
        if (esploader.transport && typeof esploader.transport.disconnect === 'function') {
          await esploader.transport.disconnect();
        }
      } catch (e) {
        appendLog('Transport disconnect Info: ' + e.message);
      }
    }

    // 2. Streams des WebSerial devices explizit auflösen
    if (device) {
      try {
        ...
        appendLog('Fehler beim Entsperren von readable: ' + e.message);
      }
      ...
      try {
        ...
        appendLog('Fehler beim Entsperren von writable: ' + e.message);
      }

      // 3. Port schließen
      try {
        await device.close();
        appendLog('COM-Port erfolgreich geschlossen.');
      } catch (e) {
        appendLog('Fehler beim Schließen des Ports: ' + e.message);
      }
    }

    // 4. State vollständig zurücksetzen
    device = null;
    transport = null;
    esploader = null;
  }
  ...
  btnFlash.addEventListener('click', async () => {
    ...
    try {
      ...
      appendLog(`Erkannter Chip: ${chipName || 'Unbekannt'} (Bootloader Offset: 0x${bootloaderOffset.toString(16)})`);

      setSummary('Flashing'); appendLog('Lade Bootloader, Partition Table und Firmware vom Server...');
      ...
      if (!bootResp.ok) throw new Error(`Bootloader-Datei nicht gefunden: ${bootFileName}`);
      if (!partResp.ok) throw new Error(`Partition-Table-Datei nicht gefunden: ${partFileName}`);
      if (!fwResp.ok) throw new Error(`Firmware-Datei nicht gefunden: ${fwFileName}`);
      ...
      appendLog(`Starte Flash-Vorgang (Bootloader @ 0x${bootloaderOffset.toString(16)}, Partition Table @ 0x${PARTITIONS_OFFSET.toString(16)} & Firmware @ 0x${FIRMWARE_OFFSET.toString(16)})...`);
      ...
      setSuccess('Bootloader, Partition Table und Firmware erfolgreich geflasht. Gerät wird neu gestartet.');
